...
(function () {
  ...
  // ========================================
  var TARGET_ENDPOINT = "api.rork.com/trpc/projects.fetchSnapshot";

  console.log(
    "[Rork Downloader] Content script loaded - listening for:",
    TARGET_ENDPOINT,
  );
  console.log(
    "[Rork Downloader] Debug: window.fetch exists =",
    typeof window.fetch,
  );
  ...
  function isTargetRequest(url) {
    ...
    if (
      url.includes("api.rork.com") &&
      url.includes("projects.fetchSnapshot")
    ) {
      console.log("[Rork Downloader] ✅ TARGET MATCHED:", url);
      return true;
    }
  ...
  function hasProjectData(data) {
    if (
      ...
      data.result.data.json.snapshot
    ) {
      return true;
    }
    ...
    if (
      ...
      data[0].result.data.json.snapshot
    ) {
      return true;
    }
    return false;
  ...
  window.fetch = function () {
    ...
    // Log requests for debugging (focus on API calls)
    if (window.__RORK_DEBUG_REQUESTS__.length < 50) {
      window.__RORK_DEBUG_REQUESTS__.push(url);
      if (url.includes("api.rork.com") || url.includes("trpc")) {
        console.log("[Rork Downloader] 📡 API Fetch:", url);
      }
    }
          ...
          clonedResponse
            ...
            .then(function (data) {
              if (isTargetRequest(url)) {
                console.log(
                  "[Rork Downloader] 🔍 Target request detected:",
                  url,
                );
                console.log(
                  "[Rork Downloader] 📦 Response structure:",
                  Object.keys(data),
                );
              }
              ...
              if (hasProjectData(data)) {
                ...
                window.dispatchEvent(
                  new CustomEvent("rork-data-captured", {
        ...
        if (shouldCheck) {
          ...
          if (contentType && contentType.includes("application/json")) {
            ...
            if (hasProjectData(data)) {
              var projectData = extractProjectData(data);
              console.log(
                "[Rork Downloader] 🎯 Captured project data via XHR!",
              );
              window.__RORK_CAPTURED_DATA__ = projectData;
                ...
                new CustomEvent("rork-data-captured", {
                  detail: {
                    ...
                    fileCount: Object.keys(
                      projectData.result.data.json.snapshot,
                    ).length,
                  },
                }),
              );
  ...
  window.addEventListener("rork-data-captured", function (e) {
    function showIndicator() {
      ...
      indicator.textContent =
        "✓ Project captured! Click extension to download.";
